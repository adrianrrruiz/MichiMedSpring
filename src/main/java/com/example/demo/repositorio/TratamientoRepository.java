...
public interface TratamientoRepository extends JpaRepository<Tratamiento, Long> {
    ...
    @Query(value = "SELECT v.NOMBRE AS veterinario_nombre, d.NOMBRE AS droga_nombre, SUM(d.UNI_VEND) AS total_unidades_vendidas FROM TRATAMIENTO t JOIN VETERINARIO v ON t.VETERINARIO_ID = v.id JOIN DROGA d ON t.DROGA_ID = d.id GROUP BY v.NOMBRE, d.NOMBRE ORDER BY total_unidades_vendidas DESC LIMIT 3", nativeQuery = true)
